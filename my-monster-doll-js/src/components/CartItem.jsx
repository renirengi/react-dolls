...
export default function CartItem({id, character, year, series, price, count, galleryImagesLinks}) {
    ...
    const onClickMinus = () => {
        count>1 ? dispatch(minusItem(id)) : onClickRemoveItem()
  ...
  {character && character.map((item, index) => (<h3 key={index}>{item}</h3>))}
    <p>{series} ({year})</p>
  </div>
